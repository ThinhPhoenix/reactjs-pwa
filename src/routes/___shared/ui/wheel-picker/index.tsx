...
import { useWheelPickerGroup, WheelPickerGroupProvider } from './ctx';
...
const WheelPickerWrapper: React.FC<WheelPickerWrapperProps> = ({
  ...
  return (
    <WheelPickerGroupProvider>
      <div className={className} data-rwp-wrapper data-vaul-no-drag>
        {children}
      </div>
    </WheelPickerGroupProvider>
  );
...
function WheelPicker<T extends WheelPickerValue>({
  ...
  const initiateDragGesture = (event: MouseEvent | TouchEvent) => {
    try {
      ...
      document.addEventListener('touchmove', handleDragMoveEvent, passiveOpts);
